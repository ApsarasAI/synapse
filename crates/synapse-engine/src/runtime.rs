...
use crate::{
    find_command, syscall_audit::collect_trace_audit_events, temp_path, SandboxCapabilities,
    SandboxEngine, SandboxError, SandboxErrorKind, SandboxExecution, SandboxFuture,
    SandboxInstance, SandboxOutput, SandboxOutputSummary, SystemProviders,
};
...
impl BubblewrapEngine {
    #[cfg(target_os = "linux")]
    pub fn probe(&self) -> Result<String, SandboxError> {
...
impl SandboxInstance for BubblewrapSandboxInstance {
    ...
    fn execute<'a>(&'a self, execution: SandboxExecution<'a>) -> SandboxFuture<'a, SandboxOutput> {
        Box::pin(async move {
            execute_binary(
                execution.runtime.binary(),
                execution.runtime.workspace_lowerdir(),
                execution.code,
                &self.upper,
...
async fn run_process(
    ...
    if let Some(execution_cgroup) = execution_cgroup.as_ref() {
        let pid = child.id().ok_or_else(|| {
            SandboxError::Execution("failed to read spawned process id".to_string())
        })?;
    ...
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| SandboxError::Execution("failed to capture stdout".to_string()))?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| SandboxError::Execution("failed to capture stderr".to_string()))?;
    ...
    match wait_result {
        ProcessOutcome::Exited(status) => {
            ...
            Ok(SandboxOutput {
                ...
                error: if memory_limit_exceeded {
                    Some(SandboxErrorKind::MemoryLimitExceeded)
                } else {
                    None
                },
                sandbox_audit,
        ...
        ProcessOutcome::WallTimeout => {
            ...
            Ok(SandboxOutput {
                ...
                duration_ms,
                output,
                error: Some(SandboxErrorKind::WallTimeout),
                sandbox_audit,
            })
        }
        ProcessOutcome::CpuTimeLimitExceeded => {
            ...
            Ok(SandboxOutput {
                ...
                duration_ms,
                output,
                error: Some(SandboxErrorKind::CpuTimeLimitExceeded),
                sandbox_audit,
            })
        }
    }
...
fn prepare_seccomp(
    ...
    sandbox_dir: &Path,
) -> Result<SeccompPlan, SandboxError> {
    match strategy {
        SandboxStrategy::Bubblewrap { .. } => {
            ...
            let filter = seccomp::export_blacklist_bpf(&filter_path).map_err(|error| {
                SandboxError::Execution(format!("failed to export seccomp profile: {error}"))
            })?;
...
fn sandbox_strategy() -> Result<SandboxStrategy, SandboxError> {
    #[cfg(target_os = "linux")]
    {
...
#[cfg(target_os = "linux")]
pub fn probe_linux_sandbox_support() -> Result<String, SandboxError> {
    BubblewrapEngine.probe()
}

#[cfg(target_os = "linux")]
fn wrap_with_strace(command: &mut Command, trace_prefix: &Path) -> Result<(), SandboxError> {
    let strace = resolve_binary("strace").map_err(|_| {
        SandboxError::Audit("strace is required for sandbox audit capture".to_string())
    })?;
...
#[cfg(target_os = "linux")]
fn detect_linux_sandbox_strategy() -> Result<SandboxStrategy, SandboxError> {
...
) -> Result<OutputCapture, SandboxError> {
    let bytes = task.await.map_err(|err| {
        SandboxError::Execution(format!("failed to collect process output: {err}"))
    })??;
...
#[cfg(unix)]
fn memory_limit_bytes(memory_limit_mb: u32) -> Result<libc::rlim_t, SandboxError> {
    let bytes = u64::from(memory_limit_mb)
        .checked_mul(1024 * 1024)
        .ok_or_else(|| SandboxError::InvalidInput("memory_limit_mb is too large".to_string()))?;
    libc::rlim_t::try_from(bytes)
        .map_err(|_| SandboxError::InvalidInput("memory_limit_mb is too large".to_string()))
}
...
fn output_summary(stdout: &OutputCapture, stderr: &OutputCapture) -> SandboxOutputSummary {
...
fn resolve_binary(binary: &str) -> Result<PathBuf, SandboxError> {
...
fn canonicalize_binary(path: &Path) -> Result<PathBuf, SandboxError> {
...
mod tests {
    ...
    use crate::{SandboxError, SandboxOutputSummary};
    use std::{
        env, fs,
        path::PathBuf,
    ...
    fn memory_limit_bytes_respects_platform_capacity() {
        ...
        if std::mem::size_of::<libc::rlim_t>() < std::mem::size_of::<u64>() {
            ...
            assert!(
                matches!(error, SandboxError::InvalidInput(message) if message == "memory_limit_mb is too large")
            );
        } else {
            assert!(result.is_ok());
        }
